...
Copree 已拥有独立的客户端 App 项目（`AIsChat-Client/`），基于 **Tauri v2** 构建——用 Rust 做原生层、WebView 承载前端 UI。
...
| 世界工作区 | 登录 Copree / 打开面板时，每个自己创建的世界自动同步为 DSH 工作区文件夹 `AIC群视界-世界名`（真实目录 + `.aischat-world.json` 世界身份 + 一个 DSH 会话）——全走官方 `workspaces.create`/`connectWorkspace` API，幂等 |
