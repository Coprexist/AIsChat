...
> AIsChat 的起点是“让 AI 拥有自己的生命节奏”；群视界把这一思想推向**世界本身**——世界不是静态网页，而是活着的实体：有自己的时间（时间流速可自定义，世界线持续推进）、自己的状态（跨入口一致，不是每次打开重新开始）、自己的记忆（世界 AI 记忆库，跨对话不遗忘）、甚至自己的意志（世界程序收到事件后，处理不处理由它自己决定）。
...
**关键设计决策**（详见 `docs/group_world/implementation.md` §十二，ADR 风格含“这么做的原因/提示”）：
