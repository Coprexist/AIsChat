...
> **服务定位**：让群聊拥有可编程的"世界"——群视界机器人创建/维护的沉浸式网页界面，前后端可自配置，世界按需懒加载
> **版本**：v0.2（设计稿 + 改动规划）
> **日期**：2026-08-04
...
| 概念 | 定义 |
|------|------|
| **世界（World）** | 独立于群聊的实体：一组网页文件 + 数据文件 + 后端代码 + 世界状态。多个群聊/私信可绑定同一个世界 |
| **群视界机器人** | 每个世界默认自带的世界 AI（群聊世界视觉界面创造机器人）；**不是 agent**——无账号、无好友关系，就是世界的一份配置（worlds.creator_config），身份 = world-{世界 id} |
| **入口** | 群聊、私信等会话是世界的访问入口，不是世界本身 |
...
**一句话**：世界是独立实体，群聊/私信只是它的入口；同一世界代码可被多个入口绑定，世界状态跨入口一致。
...
| **作用域** | 只能读写在**自己绑定入口**的数据（本世界绑定的群/私信），不可越界 |
...
- AI 绑定世界后，**到哪里都优先将环境设定为此世界**（群聊、私信一致）
...
class WorldBinding(Base):
    ...
    entity_type: str           # group | dm | user
    entity_id: int             # 群 ID / 会话 ID / 用户 ID
...
| 术语 | 定义 |
|------|------|
| **群视界机器人** | 每个世界默认自带的创造者 AI（world_agents.role='creator'，身份 = world-{世界 id}） |
