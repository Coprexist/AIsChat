...
| # | 交付 | 说明 | 依赖 |
|---|------|------|------|
| 2.1 | **py 沙箱** | ✅ **MVP 已实现**（2026-08-05）：`world_sandbox.py` subprocess + rlimit（内存默认 24MB/CPU/文件大小/进程数）+ 超时 killpg 强杀 + env 白名单（不泄漏后端密钥）；`POST /worlds/{id}/run`（仅创建者）测试入口；配额从 worlds.config 读（runtime_memory_mb/sandbox_timeout_seconds/cpu_quota）；6 项本地测试全过 | 无（最先） |
