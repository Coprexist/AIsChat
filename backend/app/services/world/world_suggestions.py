...
async def load_preset_suggestions(db: AsyncSession) -> list[str]:
...
async def suggest_fallback(db: AsyncSession, world) -> list[str]:
    ...
    try:
        ...
        if not rows:
            return await load_preset_suggestions(db)
        recent = "\n".join(
