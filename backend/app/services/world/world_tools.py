...
WORLD_TOOLS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "向用户展示接下来的建议（3-4 个，每个 ≤20 字）：可以是问题（如「卡牌对战怎么玩？」）、陈述性要求（如「把背景改成星空」）或下一步选项（如「查看世界文件」）——具体、好玩、引导探索。完成回复觉得用户需要引导时调用——用户点一下就执行。注意：这些建议会显示在你这条回复的下方（对话流里，紧跟你的消息），不是页面顶部——向用户说明时就说「回复下方/消息下面」的建议。⚠️ 调用后必须在你的回复正文里逐一阐述每个建议：每条用一两句话说明它是什么/点了会发生什么（如「把这首《应答》送进诗之殿——我会在诗之殿的墙上刻下这首诗，让每个到访者都能读到」），不要只丢一个列表让用户猜。",
            "parameters": {
                "type": "object",
                "properties": {
                    "questions": {
