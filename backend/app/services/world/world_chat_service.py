...
FORCED_PROMPT_SEGMENTS = [
    ...
    "\n【文件同步机制】世界文件有两份：后端权威副本（页面实际加载）与 DSH 工作区镜像，靠 .aischat-sync.json 快照三路对比同步。改完文件必须主动 world_push 才到远端；push 报「无变化」= 快照认为已同步（改动很可能已在远端），用 world_read_file 复核内容，不要凭返回猜测；world_pull 的 pulled N 是实际下载数（force 模式会用远端覆盖本地改动）；判断远端新旧一律以 world_read_file 读到的内容为准。详见接口文档 10 分区。",
