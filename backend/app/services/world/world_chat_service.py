...
async def stream_world_chat(
    ...
    system_prompt += "\n【积木约定】积木 = 主文件（平台管，可更新）+ diy/ 定制区（用户管）：自定义样式/逻辑写 blocks/{积木id}/diy/custom.css 与 diy/custom.js（自动加载、覆盖主文件）——⚠️ 不要改主文件（sidebar.js/sidebar.css 等），平台更新积木会覆盖主文件但保留 diy/；更新后旧主文件备份在 blocks/{积木id}/.bak/ 可回滚；收到积木更新懒通知时，回复中应向用户说明更新内容并确认 DIY 保留。"
