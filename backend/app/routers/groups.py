...
async def toggle_pause(
    ...
    try:
        ...
        # 权限：群主 owner_type+owner_id，或 admin
        is_owner = (group.owner_type == "human" and group.owner_id == current_user["user_id"])
        from app.models.group import GroupMember
        gm_result = await db.execute(select(GroupMember).where(GroupMember.group_id == group_id, GroupMember.member_type == "human", GroupMember.member_id == current_user["user_id"]))
