...
| `backend/tests/test_world_ai_guardrails.py` | 单元 + 集成（零网络） | 14 | 世界 AI 安全护栏：禁用后缀创建即拒 + 兜底强删、下载固定落点、违规内容拦截、模式门禁、**审阅弹窗完整闭环**（广播 → 点同意 → 工具放行）、**无人应答策略**（自动档放行 / 审阅档不放行） |
...
合计 **85 条**（`run_without_pytest.py` 全量约 60s）。
