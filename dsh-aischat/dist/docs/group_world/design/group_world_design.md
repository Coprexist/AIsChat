...
- **没人在看怎么办**：先等 20s（页面空闲每 10s 复查一次状态，够接上）；仍无订阅者 → 立即按「不通过」返回，不空等。用户点按钮的上限 300s，超时同样按不通过；轮次被中断也按不通过
- **弹窗内容**：标题 + 事件类型标签（下载文件/删除文件/改动世界机制/执行计划/其他事项）+ 具体内容（要写的文件路径与正文、要下载的 URL 与落点、要删的路径）
